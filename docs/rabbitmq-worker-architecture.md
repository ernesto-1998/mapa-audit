...
(Note the boundary: the SDK's in-memory buffer is best-effort and can drop events *before* they reach RabbitMQ — see SDK Architecture §7. From the moment a message is in RabbitMQ onward, this document's guarantees apply.)
...
The Worker is a standalone service (its own process/container, never embedded in a consuming app). Its loop:
